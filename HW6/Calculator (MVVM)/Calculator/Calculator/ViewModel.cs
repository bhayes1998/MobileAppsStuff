...
namespace Calculator
{
    public class ViewModel : ViewModelBase
    {
        ...
        Color backgroundColor = Color.FromHex("#3D3D3D");
        string secondButtonText = "10^x";
        string secondCommandText = "TenToTheXCommand";
        ...
        public ViewModel()
        {
            ...
            SecondCommand = new Command(
                execute: () =>
                {
                    if (BackgroundColor == Color.FromHex("#3D3D3D"))
                    {
                        BackgroundColor = Color.Red;
                        SecondButtonText = "2^x";
                        SecondCommandText = "TwoToTheXCommand";
                    }
                    else
                    {
                        BackgroundColor = Color.FromHex("#3D3D3D");
                        SecondButtonText = "10^x";
                        
                        SecondCommandText = "TenToTheXCommand";
                    }
                    RefreshCanExecutes();
                });
            TwoTenToTheXCommand = new Command(
               execute: () =>
               {
                   if (CurrentEntry[CurrentEntry.Length - 1] == '.')
                       CurrentEntry += "0";
                   ...
                   if (SecondCommandText == "TwoToTheXCommand")
                   {
                       arg = (float)Math.Pow(2, arg);

                       OutputString = OutputString.Substring(0, OutputString.Length - CurrentEntry.Length);
                       OutputString += "2^(" + CurrentEntry + ")";
                   }
                   else
                   {
        ...
        void RefreshCanExecutes()
        {
            ...
            ((Command)TwoTenToTheXCommand).ChangeCanExecute();
        }
        ...
        {
            private set { SetProperty(ref secondButtonText, value); }
            get { return secondButtonText; }
        }
        public string SecondCommandText
        {
            private set { SetProperty(ref secondCommandText, value); }
            get { return secondCommandText; }
        }
        ...
        public ICommand TwoTenToTheXCommand{ private set; get; }
        ...
        public void RestoreState(IDictionary<string, object> dictionary)
        {
            ...
            BackgroundColor = GetDictionaryEntry(dictionary, "BackgroundColor", Color.FromHex("#3D3D3D"));
            SecondButtonText = GetDictionaryEntry(dictionary, "SecondButtonText", "10^x");
            SecondCommandText = GetDictionaryEntry(dictionary, "SecondCommandText", "TenToTheXCommand");
            RefreshCanExecutes();
        }
        ...
        public void SaveState(IDictionary<string, object> dictionary)
        {
            ...
            dictionary["SecondButtonText"] = SecondButtonText;
            dictionary["SecondCommandText"] = SecondCommandText;
        }
